...
- [从 MVC 到 DDD：渐进式架构演进指南](#从-mvc-到-ddd渐进式架构演进指南)
  ...
  - [第四章：CQRS - 读写职责分离](#第四章cqrs---读写职责分离)
    ...
    - [4.7 小结](#47-小结)
- [第五章：一千个人心中的一千个 DDD](#第五章一千个人心中的一千个-ddd)
  - [5.1 从“业务规则”开始，而不是从“领域”开始](#51-从业务规则开始而不是从领域开始)
  - [5.2 实体（Entity）](#52-实体entity)
  - [5.3 值对象（Value Object）](#53-值对象value-object)
  - [5.4 聚合（Aggregate）](#54-聚合aggregate)
  - [5.5 聚合根（Aggregate Root）](#55-聚合根aggregate-root)
  - [5.6 仓储（Repository）](#56-仓储repository)
  - [5.7 应用层（Application Layer）](#57-应用层application-layer)
  - [5.8 限界上下文（Bounded Context）](#58-限界上下文bounded-context)
  - [5.9 本章小结](#59-本章小结)
- [第六章：面向演化的 DDD × 组件化开发指导文档](#第六章面向演化的-ddd--组件化开发指导文档)
<!-- /TOC -->
...
学习“领域驱动设计”的最大难关是什么？是“领域划分”在中小型项目中根本不会出现。

许多DDD教程在讲解领域划分时都会告诉你“战术”和“战略”都会成为领域划分的理由：单领域业务复杂度过高时需要划分领域，这是战术上的理由；两个开发团队分处两个城市时需要划分领域，这是战略上的理由。但在现实中，绝大多数的中小型项目都没有这两种情况：业务复杂度不高，团队规模也不大，因此根本没有领域划分的需求。对于这些项目来说，领域划分只是一个“理论上的概念”，而不是一个“实践中的问题”。因此，在学习DDD的过程中，很多开发者都会被领域划分这个概念绊倒，觉得自己学了半天DDD，但就是不知道该怎么划分领域。然后“领域驱动设计”这个名字大大挫败了他们的积极性，因为他们觉得自己领域划分划不明白，就学不好DDD。

让我们抛开这些错觉吧！既然在中小型项目中根本不需要领域划分，那不划分就是了！事实上，**“领域划分”并不是学习DDD的起点，而是复杂系统发展到一定阶段后的自然产物。**
如果你的项目规模还没有达到那个阶段，那么你完全可以先忽略它。

在本章中，我们将重新组织DDD中常见的术语，并只保留那些**真正影响代码结构的最小概念集合**。

---

## 5.1 从“业务规则”开始，而不是从“领域”开始

作为一种软件开发方法论，DDD和其他方法论面对的问题别无二致：甲方的需求。

甲方很多时候并不会直接告诉你“我需要一个订单系统”，而是会告诉你一些**业务规则**，比如：
...
如果某个程序路径允许订单总价与明细不一致，那么系统就进入了错误状态。
...
一旦你开始从这个角度思考问题，建模方式就会发生根本变化。

---

## 5.2 实体（Entity）

在实现业务规则时，我们很快会遇到这样一类对象：
...
两个 `Money` 对象如果金额和货币都相同，我们就认为它们是相同的。
...
```java
Money price = price.add(discount);
```
...
如果某个业务规则必须在一个事务中保持一致，那么相关的数据就应该放在同一个聚合中。
...
往往可以分成两个不同的聚合，因为它们可以通过最终一致性协调。

这也是为什么现代DDD实践通常提倡：

**小聚合（Small Aggregates）**

---

## 5.5 聚合根（Aggregate Root）

既然聚合中可能包含多个对象，那么系统如何保证不变量不会被破坏呢？
...
系统只能通过 `Order` 提供的方法来修改 `OrderItem`，而不能以任何方式直接修改 `OrderItem`。
...
> “聚合根”这个翻译暗示了“聚合是一个树形结构”，但实际上聚合的结构可以非常灵活，关键是要有一个明确的入口来保护不变量。

---

## 5.6 仓储（Repository）

当我们需要从数据库加载聚合时，会遇到一个问题：
...
如果你的系统规模不大，那么完全可能只有一个上下文：

```text
System = One Bounded Context
```
...
DDD并不是一套必须严格遵守的模板，而是一组帮助我们管理复杂度的工具。

# 第六章：面向演化的 DDD × 组件化开发指导文档

啊！笔者直接将自己日常使用的开发指导文档放在这里了！这份文档是笔者在多个项目中不断迭代完善的产物，让它直接作为第六章的内容再合适不过了！如果读者能以实战落地的方式来验证这篇开发指导文档的有效性，那么整篇文章的价值就完全体现了！

请跳转[第六章：面向演化的 DDD × 组件化开发指导文档](./面向演化的%20DDD%20×%20组件化开发指导文档.md)。